  ...
  const createTenantDatabase = async (req, res, next, { logger, db }) => {
    try {
      ...
      if (validatorResponse) {
        ...
        const uniqueCompanyIdCheckResponse = await TenantCreatorModel.findOne({
          company_id
        });
        if (uniqueCompanyIdCheckResponse) {
          ...
          const response = await addTenantService.createTenantDatabase(
            payload,
            logger,
            db
          );
  ...
  const createTenantSftp = async (req, res, next, { logger }) => {
    try {
      ...
      for (let i = 0; i < payload.length; i++) {
        const response = await addTenantService.createTenantSftp(
          payload[i],
          logger
        );
