...
const { S3 } = aws;
module.exports = () => {
  const createTenantDatabase = (payload, logger, db) =>
  ...
  const assignRole = (payload, logger) =>
    new Promise(async (resolve, reject) => {
      try {
        const { dbname, dbhost, dbport, collectionName, roleType } = payload;
  ...
  const createTenantSftp = (payload, logger) =>
    new Promise(async (resolve, reject) => {
      try {
